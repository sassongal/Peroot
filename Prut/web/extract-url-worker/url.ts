...
export async function extractUrl(
  ...
  const readable = await readabilityParse(html, normalizedUrl);
...
async function readabilityParse(html: string, baseUrl: string): Promise<ParsedArticle | null> {
  try {
    // Dynamic imports keep jsdom + @mozilla/readability out of the Cloudflare
    // Workers bundle (saves ~5 MiB compressed). nodejs_compat resolves them at runtime.
