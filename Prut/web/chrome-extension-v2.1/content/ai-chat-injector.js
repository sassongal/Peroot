...
(() => {
    ...
    claude: {
      ...
      getUserMessages: () =>
        document.querySelectorAll(
          '[class*="human-turn"], [data-is-streaming="false"][class*="UserMessage"]',
        ),
      getAssistantMessages: () =>
        document.querySelectorAll(
          '[class*="ai-turn"], [data-is-streaming="false"][class*="AssistantMessage"]',
        ),
    },
    gemini: {
      ...
      getUserMessages: () =>
        document.querySelectorAll('user-query, .user-query, [data-message-author="user"]'),
      getAssistantMessages: () =>
        document.querySelectorAll('model-response, .model-response, [data-message-author="model"]'),
    },
  };
  ...
  const MODE_SVGS = {
    STANDARD:
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l1.9 5.8h6.1l-4.9 3.6 1.9 5.8L12 14.6l-4.9 3.6 1.9-5.8L4 8.8h6.1z"/></svg>',
    DEEP_RESEARCH:
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2v4M12 18v4M2 12h4M18 12h4"/><circle cx="12" cy="12" r="3"/></svg>',
    IMAGE_GENERATION:
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M14.31 8l5.74 9.94M9.69 8h11.48M7.38 12l5.74-9.94M9.69 16L3.95 6.06M14.31 16H2.83M16.62 12l-5.74 9.94"/></svg>',
    VIDEO_GENERATION:
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="15" height="16" rx="2"/><path d="M17 8l5-3v14l-5-3"/></svg>',
    AGENT_BUILDER:
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M9 9h.01M15 9h.01M9 15h6"/></svg>',
  };

  const MODES = [
    {
      key: "STANDARD",
      icon: MODE_SVGS.STANDARD,
      label: "רגיל",
      proOnly: false,
    ...
    {
      key: "DEEP_RESEARCH",
      icon: MODE_SVGS.DEEP_RESEARCH,
      label: "מחקר",
      proOnly: true,
      color: MODE_COLORS.DEEP_RESEARCH,
    },
    {
      key: "IMAGE_GENERATION",
      icon: MODE_SVGS.IMAGE_GENERATION,
      label: "תמונה",
      proOnly: true,
      color: MODE_COLORS.IMAGE_GENERATION,
    },
    {
      key: "VIDEO_GENERATION",
      icon: MODE_SVGS.VIDEO_GENERATION,
      label: "וידאו",
      proOnly: true,
      color: MODE_COLORS.VIDEO_GENERATION,
    },
    {
      key: "AGENT_BUILDER",
      icon: MODE_SVGS.AGENT_BUILDER,
      label: "סוכן",
      proOnly: true,
      color: MODE_COLORS.AGENT_BUILDER,
    },
  ];
  ...
  function findInputElement() {
    ...
    for (const sel of candidates) {
      ...
      if (el && !el.closest("#peroot-side-panel, #peroot-ai-toolbar, #peroot-export-modal"))
        return el;
    }
  ...
  async function showModeSubmenu() {
    ...
    MODES.forEach((mode) => {
      ...
      item.className =
        "peroot-mode-item" + (isActive ? " active" : "") + (isLocked ? " locked" : "");
  ...
  function showPlatformSubSelector(dropdown, modeKey) {
    ...
    const currentPlatform =
      modeKey === "IMAGE_GENERATION" ? selectedImagePlatform : selectedVideoPlatform;
  ...
  const RADIAL_OPTIONS = [
    {
      id: "enhance",
      label: "שדרג",
      svg: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>',
      action: () => {
        closeRadialMenu();
        enhanceInput();
      },
    },
    {
      id: "modes",
      label: "מצבים",
      svg: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>',
      action: () => showModeSubmenu(),
    },
    {
      id: "library",
      label: "ספרייה",
    ...
    },
    {
      id: "export",
      label: "ייצוא",
      svg: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>',
      action: () => {
        closeRadialMenu();
        exportConversation();
      },
  ...
  async function loadSidePanelFavorites() {
    ...
    try {
      ...
      if (res.status === 401) {
        content.innerHTML =
          '<div class="peroot-sp-empty">\u05D4\u05EA\u05D7\u05D1\u05E8 \u05DC-Peroot \u05DB\u05D3\u05D9 \u05DC\u05D2\u05E9\u05EA \u05DC\u05DE\u05D5\u05E2\u05D3\u05E4\u05D9\u05DD</div>';
        return;
      }
      if (!res.ok) {
        content.innerHTML =
          '<div class="peroot-sp-empty">\u05E9\u05D2\u05D9\u05D0\u05D4 \u05D1\u05D8\u05E2\u05D9\u05E0\u05EA \u05DE\u05D5\u05E2\u05D3\u05E4\u05D9\u05DD</div>';
        return;
      }
      ...
      if (!favorites.length) {
        content.innerHTML =
          '<div class="peroot-sp-empty">\u05D0\u05D9\u05DF \u05DE\u05D5\u05E2\u05D3\u05E4\u05D9\u05DD \u05E2\u05D3\u05D9\u05D9\u05DF.<br>\u05E1\u05DE\u05DF \u05E4\u05E8\u05D5\u05DE\u05E4\u05D8\u05D9\u05DD \u05D1-peroot.space!</div>';
        return;
      }
      ...
      content.innerHTML =
        '<div class="peroot-sp-empty">\u05E9\u05D2\u05D9\u05D0\u05EA \u05E8\u05E9\u05EA</div>';
    }
  }
  ...
  function exportConversation() {
    ...
    if (msgElements.length > 0) {
      ...
      for (let i = 0; i < maxLen; i++) {
        if (userMsgs[i]?.innerText?.trim())
          allMsgs.push({ role: "user", text: userMsgs[i].innerText.trim() });
        if (assistantMsgs[i]?.innerText?.trim())
          allMsgs.push({ role: "assistant", text: assistantMsgs[i].innerText.trim() });
      }
    }
  ...
  function injectFixedButton() {
    ...
    trigger.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      radialOpen ? closeRadialMenu() : enhanceInput();
    });
    trigger.addEventListener("contextmenu", (e) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    fxArrow.title = "תפריט";
    fxArrow.innerHTML = "▾";
    fxArrow.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      toggleRadialMenu();
    });
