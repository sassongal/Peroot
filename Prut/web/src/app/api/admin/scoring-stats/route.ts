...
export const GET = withAdmin(async (_req, supabase) => {
  ...
  const [
    ...
    // Single query for all grouping columns — avoids 3 full-table scans
    supabase.from('history').select('capability_mode, source, input_source'),

    // Recent 50 prompts for client-side scoring sample
    supabase
      .from('history')
