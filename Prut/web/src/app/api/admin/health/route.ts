...
export async function GET() {
  try {
    ...
    // We approximate response time from time between consecutive api_usage_logs entries.
    // Instead, we count volume per hour and report cost/hour as the main signal.
    ...
    // We treat any api_usage_log entry as a successful call (it was recorded).
    // We check if last 10 entries exist; if fewer than 10 exist total, report partial.
