...
export const GET = withAdmin(async (_req, supabase) => {
  try {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const blogPostsThisWeek = (recentBlogPosts ?? []).filter((p: { created_at: string }) => {
        return p.created_at >= weekStart && p.created_at < weekEnd;
      }).length;
    ...
    const checklist = [
      ...
      },
      {
        ...
        detail:
          (blogCount ?? 0) > 0 ? `${blogCount} published posts` : "No published blog posts found",
      },
      {
        id: "public_library",
      ...
      },
      {
        ...
        status: !!gscSiteUrl,
        detail: gscSiteUrl ? `Site: ${gscSiteUrl}` : "Set GOOGLE_SEARCH_CONSOLE_SITE_URL env var",
      },
      {
    ...
    const knownRoutes = ["/", "/library", "/blog", "/prompts", "/pricing", "/about", "/admin"];
    ...
    const contentMetrics = {
      ...
      avgWeeklyContent:
        weeklyData.length > 0
          ? parseFloat((weeklyData.reduce((s, w) => s + w.total, 0) / weeklyData.length).toFixed(1))
          : 0,
    };
    ...
    const recentBlogList = (blogPosts ?? [])
      .slice(0, 5)
