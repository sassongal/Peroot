...
function SectionHeading({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <h2
      id={id}
      className="text-2xl md:text-3xl font-serif text-white mb-2 scroll-mt-24"
    >
      {children}
...
function CodeBlock({ children }: { children: string }) {
  return (
    <div className="relative group/code">
      ...
      <pre className="bg-[#0d0d0d] border border-white/10 rounded-xl p-4 pr-4 pl-10 text-sm font-mono text-slate-300 leading-relaxed overflow-x-auto whitespace-pre-wrap text-right [-webkit-overflow-scrolling:touch]" dir="rtl">
        {children}
      </pre>
    </div>
  );
...
function SidebarToc() {
  return (
    <nav aria-label="תוכן עניינים — סרגל צד" className="space-y-1">
      <p className="text-[10px] font-semibold text-amber-400 uppercase tracking-widest mb-3">תוכן עניינים</p>
      {TABLE_OF_CONTENTS.map((item, i) => (
        <a
          key={item.id}
          href={`#${item.id}`}
          className="flex items-center gap-2 py-1.5 text-[13px] text-slate-400 hover:text-amber-400 transition-colors"
        >
          <span className="w-4 h-4 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-[9px] text-slate-500 shrink-0">
            {i + 1}
          </span>
          {item.label}
        </a>
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-slate-200" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
        ...
        <section className="text-center mb-16 space-y-6 max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 bg-white/5 text-slate-400 text-xs">
              ...
              <summary className="cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                <GlassCard className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-semibold text-amber-400 uppercase tracking-widest">תוכן עניינים</p>
                    <span className="text-xs text-slate-500">({TABLE_OF_CONTENTS.length} פרקים)</span>
                  </div>
                  <ChevronDown className="w-4 h-4 text-slate-400 transition-transform group-open/toc:rotate-180" />
                </GlassCard>
              </summary>
                  ...
                  <ol className="space-y-2">
                    {TABLE_OF_CONTENTS.map((item, i) => (
                      <li key={item.id}>
                        <a
                          href={`#${item.id}`}
                          className="flex items-center gap-3 text-slate-300 hover:text-amber-400 transition-colors text-sm cursor-pointer"
                        >
                          <span className="w-5 h-5 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-[10px] text-slate-500 shrink-0">
                            {i + 1}
                          </span>
                          {item.label}
                        </a>
              ...
              {IMAGE_VIDEO_TIPS.map((tip, i) => (
                <GlassCard key={i} className="p-6 md:p-8 bg-gradient-to-br from-purple-500/[0.04] to-transparent">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-serif text-white">{tip.title}</h3>
                    </div>
                  </div>
                  <p className="text-slate-400 text-sm leading-relaxed mb-4">{tip.description}</p>
            ...
            <section className="mb-16 space-y-8">
              ...
              <GlassCard className="p-6 md:p-8 space-y-6 bg-gradient-to-br from-emerald-500/[0.03] to-transparent">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-serif text-white">מבנה System Prompt לסוכן</h3>
                    <p className="text-xs text-slate-500 mt-0.5">הגדרת תפקיד, מגבלות, סגנון ותהליך</p>
                  </div>
                </div>
            ...
            <section className="mb-16 space-y-8">
              ...
              <GlassCard className="p-6 md:p-8 space-y-6 bg-gradient-to-br from-cyan-500/[0.03] to-transparent">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-serif text-white">כיצד לבנות שאילתת מחקר</h3>
                    <p className="text-xs text-slate-500 mt-0.5">היקף + מיקוד + מבנה פלט + דרישת מקורות</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="border border-cyan-500/15 bg-cyan-950/10 rounded-xl p-4">
                    ...
                    <p className="text-sm text-slate-400 leading-relaxed">ציין תחום, טווח שנים, מיקוד גיאוגרפי ומגזר שוק.</p>
                  </div>
                  <div className="border border-cyan-500/15 bg-cyan-950/10 rounded-xl p-4">
                    <p className="text-xs font-semibold text-cyan-400 mb-1.5">2. בקש מבנה</p>
                    <p className="text-sm text-slate-400 leading-relaxed">הגדר פורמט: טבלאות, רשימות, סיכום מנהלים, השוואות.</p>
                  </div>
                  <div className="border border-cyan-500/15 bg-cyan-950/10 rounded-xl p-4">
                    <p className="text-xs font-semibold text-cyan-400 mb-1.5">3. דרוש מקורות</p>
                    <p className="text-sm text-slate-400 leading-relaxed">בקש ציון מקור לכל טענה עובדתית. ציין אם אתה צריך לינקים.</p>
                  </div>
                </div>
                <div>
                  ...
                  return (
                    <GlassCard key={i} className="p-5 bg-gradient-to-br from-amber-500/[0.03] to-transparent">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="font-serif text-white text-sm mb-1">{tip.title}</h3>
