...
function actionColor(action: string) {
  ...
  return ACTION_COLORS[key ?? ''] ?? { bg: 'bg-zinc-800/50', text: 'text-zinc-500', dot: 'bg-zinc-600' };
}
...
export default function RealtimePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      logger.error('[Realtime page] fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <div className="space-y-12 animate-in fade-in duration-1000 pb-24 select-none" dir="rtl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-zinc-950/50 p-10 rounded-[40px] border border-white/5">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {paused ? <><Play className="w-4 h-4" /> המשך</> : <><Pause className="w-4 h-4" /> השהה</>}
            </button>
        ...
        <div className="px-2 space-y-4">
          <SectionTitle icon={Activity} color="blue" title="Activity Heatmap" sub="נפח פעילות לפי שעה - 24 שעות אחרונות" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-2">
          ...
          <div className="lg:col-span-2 space-y-4">
            <SectionTitle icon={Radio} color="emerald" title="Live Feed" sub="50 האירועים האחרונים בזמן אמת" />
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isNew && "bg-white/[0.04] animate-in slide-in-from-top-1 fade-in"
                        )}
                      >
                        {/* Action dot */}
                    ...
                    <div key={u.userId} className="flex items-center gap-3">
                      ...
                      <div className="w-16 h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(u.eventCount / maxTopUser) * 100}%` }}
                        />
                      </div>
...
function CounterCard({
  ...
  const colorMap: Record<string, { icon: string; text: string }> = {
    emerald: { icon: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20', text: 'text-emerald-400' },
    blue:    { icon: 'text-blue-400 bg-blue-500/10 border-blue-500/20',          text: 'text-white'       },
    purple:  { icon: 'text-purple-400 bg-purple-500/10 border-purple-500/20',    text: 'text-white'       },
    amber:   { icon: 'text-amber-400 bg-amber-500/10 border-amber-500/20',       text: 'text-white'       },
  };
