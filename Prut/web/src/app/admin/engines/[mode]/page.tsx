...
export default function EngineEditorPage({ params }: { params: Promise<{ mode: string }> }) {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      setPlatformOverridesJson(
        po !== undefined && po !== null ? JSON.stringify(po, null, 2) : "{\n  \n}",
      );
    ...
    (async () => {
      try {
        const res = await fetch(
          getApiPath(`/api/admin/engine-shipped-baseline?mode=${encodeURIComponent(mode)}`),
        );
  ...
  const baselineDrift = useMemo(() => {
    ...
    return {
      system: norm(config.system_prompt_template) !== norm(shippedBaseline.system_prompt_template),
      user: norm(config.user_prompt_template) !== norm(shippedBaseline.user_prompt_template),
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (cm === CapabilityMode.IMAGE_GENERATION || cm === CapabilityMode.VIDEO_GENERATION) {
      ...
      <div className="space-y-10 animate-in fade-in duration-1000 select-none pb-40" dir="rtl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 bg-zinc-950/50 p-8 rounded-[40px] border border-white/5">
          <div className="flex items-center gap-8">
            <Link
              href="/admin/engines"
              className="w-16 h-16 rounded-3xl bg-zinc-900 border border-white/5 flex items-center justify-center text-zinc-500 hover:text-white hover:bg-zinc-800 transition-all active:scale-90"
            >
            ...
            <div className="space-y-2">
              ...
              <h1 className="text-4xl font-black text-white tracking-tighter">{config.name}</h1>
              <div className="text-xs font-mono text-zinc-600 uppercase">
                SYSTEM_NODE_{config.mode.toUpperCase()}
              </div>
            </div>
          </div>
        ...
        {(parseCapabilityMode(config.mode) === CapabilityMode.IMAGE_GENERATION ||
          ...
          <div
            className="p-6 rounded-[28px] border border-amber-500/20 bg-amber-500/5 text-zinc-300 text-sm leading-relaxed space-y-3"
            dir="rtl"
          >
            ...
            <p>
              התבניות למטה משמשות את מצב <strong>general</strong> בפרודקשן; לכל פלטפורמה ספציפית
              ברירת המחדל עדיין נטענת מהקוד, אלא אם כן מגדירים עקיפה ב-JSON למטה.
            </p>
          ...
          <div className="xl:col-span-7 space-y-10">
            ...
            <div className="p-8 rounded-[32px] border border-white/5 bg-zinc-950/30 space-y-6">
              ...
              <div className="space-y-4">
                <label className="block text-[10px] font-black text-zinc-700 uppercase tracking-widest mr-2">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center px-4">
                ...
                <div className="flex gap-2">
                  {systemVars.map((v) => (
                    <span
                      key={v}
                      className="px-2 py-1 rounded bg-indigo-500/10 border border-indigo-500/20 text-[9px] font-black text-indigo-400 uppercase"
                    >
                      {v}
                    </span>
                  ))}
              ...
              <div className="relative group">
                <textarea
                  ...
                  className="w-full min-h-[400px] bg-zinc-950 border border-white/5 rounded-[40px] p-10 font-mono text-sm text-zinc-400 focus:text-white focus:border-indigo-500/30 outline-none leading-relaxed resize-none shadow-inner transition-all duration-700"
                  dir="ltr"
                />
              ...
              <div className="flex justify-between items-center px-4">
                ...
                <div className="flex gap-2">
                  {userVars.map((v) => (
                    <span
                      key={v}
                      className="px-2 py-1 rounded bg-blue-500/10 border border-blue-500/20 text-[9px] font-black text-blue-400 uppercase"
                    >
                      {v}
                    </span>
                  ))}
            ...
            <div className="p-10 rounded-[40px] border border-white/5 bg-zinc-950/80 backdrop-blur-3xl sticky top-10 space-y-10 shadow-3xl overflow-hidden group">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-zinc-700 uppercase tracking-widest mr-2">
                    Simulation Input
                  </label>
                  <textarea
                    value={testInput}
                ...
                <div className="space-y-3">
                  ...
                  <div
                    className="w-full min-h-[300px] max-h-[500px] bg-black/80 border border-white/5 rounded-3xl p-8 font-mono text-xs text-zinc-500 overflow-y-auto whitespace-pre-wrap leading-relaxed shadow-inner"
                    dir="ltr"
                  >
                    {testOutput ? (
