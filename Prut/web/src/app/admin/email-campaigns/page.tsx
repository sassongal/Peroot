...
export default function EmailCampaignsPage() {
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[Email Campaigns Page] fetch error:", err);
  ...
  async function handleSend() {
    ...
    try {
      ...
      setSendResult(json);
      if (json.success) {
        // Refresh campaigns list after sending
        fetchCampaigns();
      }
    } catch (err) {
      logger.error("[Email Campaigns Page] send error:", err);
            ...
            <div className="rounded-[40px] border border-white/5 bg-zinc-950/80 backdrop-blur-3xl p-8 space-y-6 shadow-2xl">
              ...
              <div className="space-y-2">
                ...
                <div className="text-[9px] font-bold text-zinc-700">
                  {loading
                    ? "טוען נמענים..."
                    : `${fmtCount(recipientCount)} נמענים בסגמנט זה`}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          ...
          >
            ...
            <div className="rounded-2xl bg-zinc-900 border border-white/5 p-5 space-y-3">
              ...
              <DetailRow
                label="נמענים משוערים"
                value={fmtCount(recipientCount)}
                highlight
              />
            </div>
