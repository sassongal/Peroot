...
export default function FunnelPage() {
  ...
  const fetchFunnel = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[Funnel Page] fetch error:", err);
    } finally {
      setLoading(false);
    }
  }, [range]);
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {stages.map((stage, idx) => {
                ...
                const pct = prevStage
                  ? conversionPct(prevStage.count, stage.count)
                  : null;
                  ...
                  <div key={stage.key} className="space-y-2">
                    ...
                    >
                      ...
                      <div className="flex-1 relative h-10 bg-white/[0.03] rounded-xl overflow-hidden">
                        <div
                          className={cn(
                            "h-full rounded-xl bg-gradient-to-r transition-all duration-1000 shadow-lg",
                            g.from,
                            g.to,
                            g.glow
                          )}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-white/5">
                    {["From Stage", "To Stage", "Users In", "Users Out", "Conversion", "Drop-off"].map(
                      (h) => (
                        <th
                          key={h}
                        ...
                        <td className="px-8 py-5">
                          <span
                            className={cn(
                              ...
                              val >= 50 ? "text-emerald-400" : val >= 20 ? "text-amber-400" : "text-red-400"
                            )}
                          >
                            {pct}%
                          </span>
