...
export function usePromptOrdering({
  ...
  const movePrompt = useCallback(
    async (id: string, targetCategory: string, targetIndex?: number) => {
      ...
      if (user) {
            ...
            for (let i = 0; i < needShift.length; i += BATCH_SIZE) {
              ...
              await Promise.all(
                batch.map((row) =>
                  supabase
                    .from("personal_library")
                    .update({ sort_index: (row.sort_index ?? 0) + 1 })
                    .eq("id", row.id)
                    .eq("user_id", user.id),
                ),
              );
        ...
        setAllLocalItems((prev) => {
          ...
          return without
            .map((i) => reindexedMap.get(i.id) ?? i)
            .concat(reindexedMap.get(id) as PersonalPrompt);
        });
      }
    },
  ...
  const addPrompts = useCallback(
    async (prompts: Omit<PersonalPrompt, "id" | "use_count" | "created_at" | "updated_at">[]) => {
      ...
      if (user) {
        ...
        const existingTexts = new Set(
          (existingRows ?? []).map((r: { prompt: string }) => r.prompt.trim()),
        );
        ...
        const categoriesInBatch = Array.from(
          new Set(uniquePrompts.map((p) => p.personal_category ?? "כללי")),
        );
        ...
        const categoriesInBatch = Array.from(
          new Set(newItems.map((item) => item.personal_category)),
        );
        ...
        setAllLocalItems((prev) => {
          ...
          const assignedItems = newItems.map((item) => ({
            ...item,
            sort_index: indexAssignments.has(item.id)
              ? indexAssignments.get(item.id)!
              : item.sort_index,
          }));
