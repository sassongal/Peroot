...
export function usePromptMutations({
  ...
  const updatePrompt = useCallback(
    async (id: string, updates: Partial<PersonalPrompt>) => {
    ...
    async (id: string, success: boolean) => {
      ...
      if (user) {
        const { data: current } = await supabase
          ...
          .eq("user_id", user.id)
          .single();
        const currentVal = success ? (current?.success_count ?? 0) : (current?.fail_count ?? 0);
        ...
        setPersonalLibrary((prev) =>
          prev.map((p) =>
            p.id === id
              ? { ...p, [field]: ((p[field as keyof PersonalPrompt] as number) ?? 0) + 1 }
              : p,
          ),
        );
      } else {
  ...
  const incrementUseCount = useCallback(
    async (id: string) => {
      ...
      if (user) {
        await supabase
          .rpc("increment_use_count", { item_id: id, user_id_input: user.id })
          .then(async (res) => {
            if (res.error) {
              const { data: current } = await supabase
                .from("personal_library")
        ...
        setPersonalLibrary((prev) =>
          prev.map((p) =>
            p.id === id
              ? { ...p, use_count: p.use_count + 1, updated_at: now, last_used_at: now }
              : p,
          ),
        );
      } else {
  ...
  const updatePromptContent = useCallback(
    async (id: string, prompt: string, prompt_style?: string) => {
      ...
      if (user) {
        ...
        await supabase.from("personal_library").update(updates).eq("id", id).eq("user_id", user.id);
        await refreshCurrentPage();
      } else {
        setAllLocalItems((prev) =>
          prev.map((p) =>
  ...
  const bumpPersonalLibraryLastUsed = useCallback(
    (id: string): void => {
      if (!user) return;
      void supabase
