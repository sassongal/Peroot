...
interface LibraryDataContextType {
  ...
  /** Full in-memory guest corpus (empty for server users) — for graph/palace. */
...
export function LibraryDataProvider({
  ...
  const suggestCategory = useCallback(
    async (promptId: string, promptText: string, categories: string[]) => {
      try {
  ...
  const addPromptWithSuggestion = useCallback(
    async (
      prompt: Omit<PersonalPrompt, "id" | "created_at" | "updated_at" | "use_count">,
      category?: string,
    ): Promise<string | undefined> => {
  ...
  const deletePersonalCategory = useCallback(
    async (categoryName: string, mode: "move" | "delete" = "move") => {
      try {
        if (!user) {
        ...
        toast.success(
          mode === "delete" ? "התיקייה והפרומפטים נמחקו" : "התיקייה נמחקה, הפרומפטים הועברו לכללי",
        );
      } catch (e) {
  ...
  const saveRenameCategoryWrapped = useCallback(
