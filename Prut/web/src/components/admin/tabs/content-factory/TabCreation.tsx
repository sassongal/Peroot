    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          {cronSettings.enabled ? (
            <>
              ...
              <span className="text-zinc-400">
                ריצה הבאה: יום {DAYS_SHORT[cronSettings.day]} {nextCronDate(cronSettings.day)}
              </span>
            </>
          ) : (
            <>
            ...
            {pending.map((item) => (
              ...
              >
                ...
                {item.type === "prompt" && item.batch_id && (
                  <button
                    onClick={() =>
                      approve([item.id, ...(item.batch_items?.map((b) => b.id) ?? [])], "prompt")
                    }
      ...
      {previewItem && (
        <PreviewModal
          ...
          fullData={rawPendingData[previewItem.id]}
        />
      )}
    </div>
  );
