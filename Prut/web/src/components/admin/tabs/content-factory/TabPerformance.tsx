...
import { PenTool, Zap, BarChart2, AlertTriangle } from "lucide-react";
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              {!stats?.topBlogPosts || stats.topBlogPosts.length === 0 ? (
                ...
                stats.topBlogPosts.map((p, i) => (
                  <tr key={p.id} className="hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {p.category ? (
                        <span className="text-[9px] font-black px-2.5 py-1 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400">
                          {p.category}
                        </span>
                      ) : (
      ...
      <div className="space-y-4">
        <SectionTitle
          icon={BarChart2}
          color="emerald"
          title="איזון קטגוריות"
          sub="Content distribution across categories"
        />
                ...
                <div key={cat.name} className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-black text-zinc-500 tabular-nums">
                      {cat.count}
                      {cat.target > 0 && <span className="text-zinc-700"> / {cat.target}</span>}
                    </span>
                  </div>
      ...
      <div className="space-y-4">
        <SectionTitle
          icon={AlertTriangle}
          color="amber"
          title="תוכן ללא מעורבות"
          sub="Items with 0 engagement after 30 days"
        />
