  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="text-[9px] font-bold text-zinc-600 uppercase tracking-wider mt-0.5">
          {sub}
        </div>
      </div>
      <div
...
export function HealthTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[Health tab] fetch error:", err);
    } finally {
      setLoading(false);
    }
  }, []);
          ...
          <div className="p-8 rounded-[36px] bg-zinc-950/80 border border-white/5 backdrop-blur-3xl space-y-6">
            {loading && !data ? (
              ...
              <>
                ...
                {data && <StatusBadge status={data.errorRate.status} />}
              </>
            )}
          </div>
        </div>
              ...
              <>
                <div className="space-y-4">
                  {[
                    {
                      label: "היום",
                      value: data?.costBurnRate.today ?? 0,
                      highlight: true,
                    },
                    {
                      label: "אתמול",
                      value: data?.costBurnRate.yesterday ?? 0,
                      highlight: false,
                    },
                    {
                      label: "ממוצע 7 ימים",
                      value: data?.costBurnRate.sevenDayAvg ?? 0,
                      highlight: false,
                    },
                  ].map((row) => {
                ...
                {data && (
                  <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                    ...
                    >
                      {data.costBurnRate.trend > 0 ? "+" : ""}
                      {data.costBurnRate.trend.toFixed(1)}% vs אתמול
                    </span>
                  </div>
                )}
              </>
