...
function SummaryCard({
  ...
  return (
    <div className="group p-7 rounded-[36px] bg-zinc-950/80 border border-white/5 flex flex-col gap-5 transition-all duration-500 hover:border-white/10 hover:shadow-2xl backdrop-blur-sm">
      <div
        className={cn(
          "p-3 rounded-2xl border self-start transition-all duration-500",
          colors[color],
        )}
      >
...
function DonutChart({ data }: { data: ActionsByType[] }) {
  ...
  const segments = top8.reduce<
    Array<(typeof top8)[number] & { dashArray: number; dashOffset: number; color: string }>
  >((acc, item, idx) => {
      ...
      <div className="space-y-2">
        {segments.map((seg, i) => (
          <div key={i} className="flex items-center gap-3 group">
            ...
            <span className="text-[10px] font-black text-zinc-600 tabular-nums">{seg.count}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      {data.map((day, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-2 group min-w-0">
          <div className="w-full relative flex items-end" style={{ height: "140px" }}>
            <div
              className="w-full bg-linear-to-t from-indigo-700 to-indigo-500 rounded-t-lg transition-all duration-700 group-hover:from-indigo-500 group-hover:to-white group-hover:shadow-[0_0_16px_rgba(100,104,212,0.4)]"
              style={{
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20" dir="rtl">
        ...
        <div className="flex flex-col gap-1.5 flex-1 min-w-48">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-white/5 text-white rounded-2xl pr-10 pl-4 py-2.5 text-sm font-bold placeholder:text-zinc-700 focus:outline-none focus:border-indigo-500/40 transition-colors"
            />
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={handleApply}
            disabled={loading}
            className="px-8 py-2.5 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="rounded-[36px] border border-white/5 bg-zinc-950/80 backdrop-blur-3xl p-8 shadow-2xl space-y-6">
          <SectionTitle icon={Zap} color="blue" title="לפי סוג פעולה" sub="Actions by Type" />
          {loading ? (
            <div className="flex items-center justify-center h-48">
              ...
              return (
                <div key={admin.user_id} className="flex items-center gap-5 group">
                  ...
                  <div className="w-44 shrink-0">
                    ...
                    <div className="text-[9px] text-zinc-700 font-bold truncate">{admin.email}</div>
                  </div>
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          <td className="px-8 py-5">
                            <div className="flex flex-col gap-0.5">
                              <span className="text-sm font-black text-zinc-300 group-hover:text-white transition-colors">
                                {log.admin_name !== "Unknown Admin" ? log.admin_name : "Admin"}
                              </span>
                          ...
                          {/* Action */}
                          <td className="px-8 py-5">
                            <span className="px-3 py-1.5 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-wider whitespace-nowrap">
                              {log.action}
                            </span>
                          </td>
