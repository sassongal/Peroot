...
export function ActivityFeedTab() {
  ...
  function getActionColor(action: string) {
    ...
    if (a.includes("create") || a.includes("הוספ") || a.includes("חדש"))
      return "text-emerald-400 bg-emerald-500/10 border-emerald-500/20";
    if (a.includes("delete") || a.includes("מחק") || a.includes("חסום"))
      return "text-rose-400 bg-rose-500/10 border-rose-500/20";
    if (a.includes("update") || a.includes("עדכון") || a.includes("שינוי"))
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-400 font-medium tracking-wide">
            ניטור מלא של כל הטרנזקציות ומעשי המנהלים במערכת
          </p>
        </div>
      ...
      <div className="rounded-[32px] border border-white/10 bg-zinc-900/30 backdrop-blur-3xl overflow-hidden shadow-2xl">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-4">
            ...
            <span className="text-slate-600 font-black uppercase tracking-widest text-xs">
              Fetching Audit Logs...
            </span>
          </div>
          ...
          <div className="divide-y divide-white/5">
            {filteredLogs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="group p-6 hover:bg-white/3 transition-all duration-300"
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-6">
                    <div
                      className={cn(
                        "p-4 rounded-[22px] border transition-transform group-hover:scale-110",
                        colorClasses,
                      )}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-slate-500">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>{formatDateHe(log.created_at)}</span>
                        </div>
                        <div className="w-1 h-1 rounded-full bg-slate-800" />
                        <div className="flex items-center gap-2">
                      ...
                      {log.details && Object.keys(log.details).length > 0 && (
                        <div className="mt-4 p-4 rounded-2xl bg-black/40 border border-white/5 font-mono text-[10px] text-emerald-500/80 overflow-x-auto leading-relaxed">
                          <div className="flex justify-between items-center mb-2 border-b border-white/5 pb-2">
                            <span className="text-slate-700 font-black uppercase tracking-tighter">
                              Extended Data
                            </span>
                            <span className="text-[8px] text-slate-800">
                              OBJECT_ID: {log.id.slice(0, 8)}
                            </span>
                          </div>
                    ...
                    <div className="flex items-center justify-end">
                      <button
                        ...
                        className="p-3 bg-white/5 hover:bg-white/10 rounded-xl text-slate-500 transition-all opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0"
                      >
