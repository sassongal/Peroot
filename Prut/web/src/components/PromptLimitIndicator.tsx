  ...
  if (isAdmin) {
    return (
      <div className="flex items-center gap-1 px-2 py-1.5 rounded-lg bg-blue-500/5 dark:bg-black/40 border border-blue-500/30 backdrop-blur-md">
        ...
        <span className="text-[10px] font-semibold text-blue-300 tracking-wide hidden sm:inline">
          Admin
        </span>
      </div>
    );
  ...
  if (!isGuest) {
    ...
    if (isEmpty) {
      return (
        <Link
          href="/pricing"
          className="flex items-center gap-2 px-3 py-2 min-h-[44px] rounded-xl border border-red-500/50 bg-red-500/10 backdrop-blur-md transition-all hover:bg-red-500/20 animate-pulse"
        >
    ...
    >
      ...
      <span className="text-xs font-medium text-[var(--text-primary)]">
        ...
        <span className="text-[var(--text-muted)] font-normal hidden md:inline">
          {" "}
          {settings.allow_guest_access ? "פרומפטים היום" : "נדרשת הרשמה"}
        </span>
      </span>
    </Link>
  );
