...
export function LoadingOverlay({ isVisible, message = "משפר את הפרומפט שלך..." }: LoadingOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md animate-in fade-in duration-300 overscroll-contain" aria-live="polite">
      <div className="relative flex flex-col items-center gap-8 p-16 rounded-3xl bg-black/60 border border-white/10 shadow-2xl max-w-md text-center">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-amber-500/10 via-yellow-500/10 to-orange-500/10 blur-3xl rounded-full animate-pulse" />
