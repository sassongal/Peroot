...
export default function StreamingProgress({ phase, onStop }: StreamingProgressProps) {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="flex items-center justify-center gap-2.5 text-sm text-amber-600/80 dark:text-amber-400/80 py-3"
    >
      ...
      {onStop && (
        <button
          type="button"
          onClick={onStop}
          className="ms-3 px-3 py-1 min-h-[32px] rounded-lg border border-(--glass-border) text-xs text-(--text-secondary) hover:text-red-500 hover:border-red-500/40 transition-colors cursor-pointer"
        >
          עצור
        </button>
      )}
