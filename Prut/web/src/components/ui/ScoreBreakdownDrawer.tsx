...
export function ScoreBreakdownDrawer({
  ...
  const levelColor = {
    ...
    elite: "text-indigo-600 dark:text-indigo-300 bg-indigo-500/10 border-indigo-500/30",
  }[score.level];
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 p-4 md:p-6 pb-4 border-b border-(--glass-border)">
          <div className="space-y-2 min-w-0 flex-1">
            <h2 className="text-xl md:text-2xl font-bold text-(--text-primary) wrap-break-word">
              {title}
            </h2>
              ...
              <div className="space-y-1.5">
                ...
                <ul className="space-y-1">
                  {score.strengths.slice(0, 3).map((s, i) => (
                    <li
                      key={i}
                      className="text-xs text-emerald-600 dark:text-emerald-300 flex items-start gap-1.5"
                    >
