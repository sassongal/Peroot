...
export default function UpgradeNudge({ type, remaining, resetDate, onUpgrade, onDismiss }: UpgradeNudgeProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-slate-900 border border-white/10 rounded-xl p-6 max-w-sm mx-4 space-y-4 shadow-xl">
        ...
        <div className="flex flex-col gap-2">
          <button
            onClick={onUpgrade}
            className="w-full py-2.5 rounded-lg bg-white text-black font-medium min-h-11"
          >
            {t.nudge?.upgrade_to_pro || 'שדרג ל-Pro'}
          </button>
          <button
            onClick={onDismiss}
            className="w-full py-2.5 rounded-lg border border-white/10 text-slate-400 text-sm min-h-11"
          >
            {resetDate
