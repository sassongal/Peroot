...
export function LibraryView({ onUsePrompt, onCopyText }: LibraryViewProps) {
  ...
  const uniqueCategories = useMemo(() => {
    ...
    collectionFilteredLibrary.forEach((p) => {
      ...
      const key = CATEGORY_LABELS[raw]
        ? raw
        : CATEGORY_LABELS[raw.charAt(0).toUpperCase() + raw.slice(1)]
  ...
  return (
    <div
      id="library-top"
      className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-full overflow-x-hidden pb-20 md:pb-0 w-full"
    >
      {/* Back Button */}
      <button
      ...
      <div className="glass-card p-3 md:p-5 lg:p-6 rounded-2xl border-(--glass-border) bg-black/40">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h2 className="text-lg md:text-2xl font-serif heading-title-gradient page-heading-glow heading-enter">
              ספריית פרומפטים
            </h2>
          ...
          >
            <div className="relative w-5 h-5">
              ...
              <Plus
                className="absolute inset-0 w-5 h-5 text-black translate-x-0.5 translate-y-0.5"
                strokeWidth={2.5}
              />
            </div>
        ...
        <div className="flex items-center gap-2 mt-3 overflow-x-auto scrollbar-hide pb-1">
          ...
          >
            <Star
              className={cn(
                "w-4 h-4",
                libraryView === "favorites" && "fill-yellow-200 text-yellow-200",
              )}
            />
            מועדפים
          </button>
        ...
        <div className="mt-3">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[11px] md:text-xs font-medium text-(--text-muted)">
              חבילות פרומפטים
            </span>
            {activeCollection && (
              <button
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {PROMPT_COLLECTIONS.map((collection) => (
              ...
              >
                {(() => {
                  const IconComponent = COLLECTION_ICONS[collection.icon];
                  return IconComponent ? (
                    <IconComponent className="w-5 h-5 md:w-6 md:h-6 text-(--text-secondary) mb-1" />
                  ) : null;
                })()}
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 md:gap-2 p-1.5 md:p-2 rounded-xl md:rounded-2xl border border-(--glass-border) bg-[#0A0A0A]/95 backdrop-blur-xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 w-[calc(100%-1rem)] md:w-auto max-w-md md:max-w-none">
          ...
          <button
            ...
            className="p-1 hover:bg-black/5 dark:bg-white/10 rounded-full text-(--text-muted)"
            aria-label="סגור"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-[#111] border border-(--glass-border) rounded-2xl p-6 w-full max-w-sm shadow-2xl mx-4 max-h-[80vh] overflow-y-auto">
            <h3 className="text-xl text-(--text-primary) font-serif mb-4 text-center">
              שמירת {selectedIds.size} פריטים
            </h3>
            <p className="text-(--text-muted) text-sm mb-6 text-center">
              בחר לאיזו קטגוריה לשמור בספריה האישית שלך
            </p>
      ...
      {isLibraryFetching && totalCount === 0 && (
        <div className="space-y-3">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-xl bg-white/4 animate-pulse h-16" />
          ))}
        </div>
      )}
