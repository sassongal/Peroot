    ...
    <>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 p-2 rounded-2xl border border-(--glass-border) bg-[#0A0A0A]/95 backdrop-blur-xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 w-[calc(100%-2rem)] md:w-auto">
          ...
          <button onClick={() => setShowMoveDialog(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-black/5 dark:bg-white/10 text-(--text-secondary) text-xs transition-colors focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none">
            <FolderInput className="w-4 h-4" /> <span className="hidden md:inline">העבר ל...</span>
          </button>
          <button onClick={() => setShowTagDialog(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-black/5 dark:bg-white/10 text-(--text-secondary) text-xs transition-colors focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none">
            <Tag className="w-4 h-4" /> <span className="hidden md:inline">תגיות</span>
          </button>
          <button onClick={handleBatchExport} className="flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-black/5 dark:bg-white/10 text-(--text-secondary) text-xs transition-colors focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none">
          ...
          <button onClick={clearSelection} className="p-1.5 hover:bg-black/5 dark:bg-white/10 rounded-full text-(--text-muted) focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none" aria-label="סגור">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      {showMoveDialog && (
        <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-[#111] border border-(--glass-border) rounded-2xl p-6 w-full max-w-sm shadow-2xl mx-4" dir="rtl">
            <h3 className="text-xl text-(--text-primary) font-serif mb-4 text-center">העברת {selectedIds.size} פריטים</h3>
            <div className="space-y-2 mb-6 max-h-64 overflow-y-auto">
              <button
                ...
                className={cn("w-full text-start px-4 py-3 rounded-xl border transition-all text-sm flex items-center justify-between focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none",
                  isCreatingNewMoveCategory ? "bg-blue-600/20 border-blue-500 text-blue-200" : "bg-(--glass-bg) border-(--glass-border) text-(--text-secondary) hover:bg-black/5 dark:bg-white/10"
                )}
              >
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setShowMoveDialog(false); setIsCreatingNewMoveCategory(false); }} className="flex-1 bg-(--glass-bg) text-(--text-secondary) py-2.5 rounded-lg focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none">ביטול</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showTagDialog && (
        <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-[#111] border border-(--glass-border) rounded-2xl p-6 w-full max-w-sm shadow-2xl mx-4" dir="rtl">
            <h3 className="text-xl text-(--text-primary) font-serif mb-4 text-center">הוספת תגיות</h3>
            <p className="text-(--text-muted) text-sm mb-4 text-center">הזן תגיות מופרדות בפסיקים</p>
            <input
              value={tagsInput}
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowTagDialog(false)} className="flex-1 bg-(--glass-bg) text-(--text-secondary) py-2.5 rounded-lg focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:outline-none">ביטול</button>
            </div>
          </div>
        </div>
      )}
      ...
      {folderContextMenu && (
        ...
        >
          <button onClick={() => handleFolderRename(folderContextMenu.folder)} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-(--text-secondary) hover:bg-black/5 dark:bg-white/10 hover:text-(--text-primary)">
