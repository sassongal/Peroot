...
describe("StandardEngine", () => {
  ...
  it("systemPrompt contains GENIUS_QUESTIONS section (refinement flow)", () => {
    const result = engine.generateRefinement(makeInput({ previousResult: "פרומפט קודם לבדיקה" }));
...
describe("All engines - common behavior", () => {
  ...
  for (const { name, engine, mode } of engines) {
    describe(name, () => {
      ...
      it("emits GENIUS_QUESTIONS marker (refinement flow universally; some engines also in generate)", () => {
        // Image/Video/Agent emit GENIUS_QUESTIONS in generate(); Standard/Research
        // only inject it in generateRefinement(). Refinement is the universal
        // contract, so we lock in that path here.
        const result = engine.generateRefinement(
          makeInput({ mode, previousResult: "פרומפט קודם לבדיקה" }),
        );
        expect(result.systemPrompt).toContain("GENIUS_QUESTIONS");
      });
