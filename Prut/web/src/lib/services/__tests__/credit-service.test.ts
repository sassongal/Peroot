...
describe('CreditService', () => {
  ...
  describe('checkAndDecrementCredits', () => {
    ...
    it('handles free tier daily reset via legacy fallback', async () => {
      // First RPC call: refresh_and_decrement_credits does not exist
      const rpcFn = vi.fn()
        // 1st call: modern RPC -> function not found
        .mockResolvedValueOnce({
          data: null,
          error: { message: 'function refresh_and_decrement_credits(unknown, unknown, unknown) does not exist' },
        })
        // 2nd call: check_and_decrement_credits -> success
        .mockResolvedValueOnce({
      ...
      // Verify the legacy fallback RPC was called
      expect(rpcFn).toHaveBeenCalledTimes(2);
      expect(rpcFn).toHaveBeenNthCalledWith(2, 'check_and_decrement_credits', {
  ...
  describe('atomic decrement prevents negative credits', () => {
    it('two concurrent calls both get the RPC result (DB atomicity)', async () => {
      ...
      const client2 = { ...client1, rpc: rpcFn } as unknown as import('@supabase/supabase-js').SupabaseClient;
  ...
  describe('credit ledger entry created on decrement', () => {
    it('calls logCreditChange after successful decrement', async () => {
      const client = buildQueryClient({
        rpc: mockRpc({ success: true, current_balance: 9 }),
      });

      // serviceClientMock is used by logCreditChange internally
      serviceClientMock = buildQueryClient({
        rpc: mockRpc(null, null), // log_credit_change RPC
      });

      await checkAndDecrementCredits('user-1', 'pro', client);

      // logCreditChange fires async (fire-and-forget), give it a tick
      await vi.waitFor(() => {
        expect(serviceClientMock.rpc).toHaveBeenCalledWith('log_credit_change', {
          p_user_id: 'user-1',
          p_delta: -1,
          p_balance_after: 9,
          p_reason: 'spend',
    ...
    it('does not block the response if ledger logging fails', async () => {
      const client = buildQueryClient({
        rpc: mockRpc({ success: true, current_balance: 5 }),
      });

      // Service client throws on the ledger RPC
      serviceClientMock = buildQueryClient({
        rpc: vi.fn().mockRejectedValue(new Error('ledger DB down')),
      });

      // Should still return successfully -- ledger is best-effort
      const result = await checkAndDecrementCredits('user-1', 'pro', client);
      expect(result.allowed).toBe(true);
  ...
  describe('refundCredit', () => {
    ...
    it('handles non-existent user gracefully (no throw)', async () => {
      ...
      await expect(refundCredit('nonexistent-user')).resolves.toBeUndefined();
    });
  ...
  describe('credit ledger entry created on refund', () => {
    ...
    it('skips ledger if profile fetch returns null', async () => {
      const rpcFn = vi.fn()
        .mockResolvedValueOnce({ data: null, error: null }); // refund_credit
  ...
  describe('logCreditChange', () => {
    ...
    it('defaults source to "system"', async () => {
      ...
      expect(rpcFn).toHaveBeenCalledWith('log_credit_change', expect.objectContaining({
        p_source: 'system',
      }));
    });
  ...
  describe('adminAdjustCredits', () => {
    it('grants credits via admin_adjust_credits RPC', async () => {
      const rpcFn = vi.fn().mockResolvedValueOnce({ data: null, error: null })
  ...
  describe('legacy fallback path', () => {
    it('legacy check_and_decrement_credits failure returns correct shape', async () => {
      const rpcFn = vi.fn()
        // Modern RPC: not found
        .mockResolvedValueOnce({
          data: null,
          error: { message: 'function refresh_and_decrement_credits does not exist' },
        })
        // Legacy RPC: fails
        .mockResolvedValueOnce({
