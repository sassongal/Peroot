...
describe("extractVariables", () => {
  ...
  it("accepts hyphen and single-space multi-word keys", () => {
    expect(extractVariables("{target audience} and {tone-style}")).toEqual([
      "target audience",
      "tone-style",
    ]);
  });
...
describe("substituteVariables", () => {
  it("substitutes a single token", () => {
    expect(substituteVariables("Hello {name}", { name: "Gal" })).toBe("Hello Gal");
  });
  ...
  it("leaves a token untouched when its value is missing or empty", () => {
    expect(substituteVariables("A {a} B {b} C", { a: "ONE" })).toBe("A ONE B {b} C");
  ...
  it("does not touch JSON-shaped braces inside the text", () => {
    ...
    expect(substituteVariables(text, { brand_name: "Peroot" })).toBe(
      `Use JSON: { "k": "v" } and fill Peroot`,
    );
  });
