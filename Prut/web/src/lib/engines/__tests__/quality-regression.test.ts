...
describe("EnhancedScorer, dimension behaviour", () => {
  ...
  it("text mode exposes 15 dimensions, visual mode exposes 7-8 visual ones", () => {
    ...
    const imageScore = EnhancedScorer.score(
      "a portrait of a woman",
      CapabilityMode.IMAGE_GENERATION,
    );
    const videoScore = EnhancedScorer.score(
      "a portrait of a woman walking",
...
describe("EnhancedScorer, anti-gaming penalties", () => {
  ...
  it("penalizes contradictions: brevity + high word count", () => {
    ...
    const safetyBase =
      "מחוץ לתחום: לא לעסוק בפוליטיקה. מקרה קצה: אם המידע חסר, אם אין נתון אז ציין.";
  ...
  it("rewards task-relevant numbers more than free-floating numbers in specificity", () => {
    const taskRelevant =
      'אתה מומחה. כתוב 200 מילים על הנושא, ב-5 נקודות. למשל: "כותרת". Apple Inc.';
