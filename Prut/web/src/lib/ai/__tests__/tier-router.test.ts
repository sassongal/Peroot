...
describe("selectModelByLength", () => {
  ...
  it("routes long prompts to the primary flash", () => {
    expect(selectModelByLength(200)).toBe("gemini-3-flash");
    expect(selectModelByLength(5000)).toBe("gemini-3-flash");
  });
  it("respects custom threshold", () => {
    expect(selectModelByLength(150, 100)).toBe("gemini-3-flash");
