...
describe("gateway buildProviderOptions — disables Gemini thinking", () => {
  ...
  it("bounds thinking to 1024 for enhance task (reasoning stays on, capped)", () => {
    expect(buildProviderOptions("enhance")).toEqual({
      google: { thinkingConfig: { thinkingBudget: 1024 } },
    });
  });

  it("bounds thinking to 2048 for research task", () => {
    expect(buildProviderOptions("research")).toEqual({
      google: { thinkingConfig: { thinkingBudget: 2048 } },
    });
  });

  it("bounds thinking to 2048 for agent task", () => {
    expect(buildProviderOptions("agent")).toEqual({
      google: { thinkingConfig: { thinkingBudget: 2048 } },
    });
  });

  it("defaults to 1024 thinking budget for unknown task", () => {
    expect(buildProviderOptions("bogus-task-name")).toEqual({
      google: { thinkingConfig: { thinkingBudget: 1024 } },
    });
  });

  it("defaults to 1024 thinking budget when task is missing", () => {
...
describe("filterModelsForEstimatedInput", () => {
  ...
  it("drops gpt-oss-20b when estimated input exceeds safe window for 32k context", () => {
    const chain = ["gemini-2.5-flash", "mistral-small", "gpt-oss-20b"] as const;
